...
本仓库是一个 **Claude Code plugin**（`.claude-plugin/` 清单 + 内含 1 个 skill `shandong-cee-line-diff`），同时可被 zcode / Codex / npx 等方式安装。各 agent 用**各自的原生方式**：

### Claude Code（原生 plugin，推荐）

```bash
claude plugin marketplace add ChHsiching/shandong-cee-line-diff
claude plugin install shandong-cee-line-diff@shandong-cee-line-diff
```

或 Claude Code 内 `/plugin marketplace add ChHsiching/shandong-cee-line-diff` → `/plugin install shandong-cee-line-diff@shandong-cee-line-diff`。skill 随 plugin 自动注册，`Skill("shandong-cee-line-diff")` 即可调用，**无需手动复制脚本**。

### ZCode（智谱）—— 一条命令

ZCode 没有 CLI 安装命令（只能把 skill 文件放进 `~/.zcode/skills/`），用安装器自动铺平安装（含管线脚本，自包含可跑）：

```bash
curl -fsSL https://raw.githubusercontent.com/ChHsiching/shandong-cee-line-diff/main/install.sh | bash
```

或 ZCode 桌面应用：**设置 → 技能 → 导入** → 自动扫描外部 agent 的 skill 目录 → 一键导入。
...
skill 是 playbook；跑管线需要 `scripts/` + `data/` 在工作目录。最简：克隆本仓库作为工作目录，放进数据，调用 skill：

```bash
git clone https://github.com/ChHsiching/shandong-cee-line-diff.git
cd shandong-cee-line-diff
python3 -m venv .venv && .venv/bin/pip install openpyxl pytest ruff
# 把当年三个原始 xlsx 放进 data/（见 data/README.md），再调用 Skill("shandong-cee-line-diff")
```

> ZCode 用户经 `install.sh` 安装时，scripts 已随 skill 装进 `~/.zcode/skills/shandong-cee-line-diff/`，直接把数据放进该 `data/` 即可，无需另克隆。
...
```
shandong-cee-line-diff/    # Claude Code plugin（含 1 个 skill）+ 参考实现
├── .claude-plugin/        # plugin 清单：plugin.json + marketplace.json
├── skills/
│   └── shandong-cee-line-diff/   # skill 本体（plugin 内）
│       ├── SKILL.md               # skill 入口（自主启动流程 + 铁律 + 匹配规则）
│       └── REFERENCE.md           # 管线阶段 / 审计 / 故障排查（按需查阅）
├── install.sh             # ZCode 一键安装器（curl 管道）
...
- [x] README 完整；skill 存在（`skills/shandong-cee-line-diff/SKILL.md`）。
...
可复用的核心步骤已沉淀为 skill（`skills/shandong-cee-line-diff/SKILL.md`）
