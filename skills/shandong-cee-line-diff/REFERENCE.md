...
- **扩充忽略类**（匹配/复核用，补默认集）：章程/就业/面向/详见/不招/只招/单色识别/校区/办学地点/
  培养模式/学费/中外/国际交流/转专业/缴/人民币/欧元/高校专项计划/地方专项计划/综合评价招生
  （描述性、章程引用、培养细节——非差异化，剥除后再比较）。
- `direction_normalized(name)`：剥扩充忽略类后，各方向括号去标点→字符排序集合；两专业此集合相等 =
  方向归一化同一（构造确定，匹配、**不进复核存疑**）。消化「5+3一体化,儿科学」↔「5+3一体化儿科学」、
  「智能语言实验班」↔「语言智能实验班」等标点/词序/措辞差异。
- `direction_subset(a,b)`：a 某方向括号内容含/被含 b 某方向 = 拆括号/子串同一（去年1→今年多=同一批，
  如图灵班↔图灵班(智算/智能可信方向)、金融学↔金融学(两方向)）。
...
- **方向归一化/子串匹配优先（构造确定，不进复核）**：候选先过 `direction_normalized` 相等
  或 `direction_subset` → 直接接受（消化标点/词序/拆括号差异，945 例实证）；仅都不满足才进括号子集消歧。
- 唯一候选 → 自动接受（签名全等禁用，实测 0%）；多候选 → 括号子集消歧
  （候选每个 diff_bracket 均是大绿本全名子串 → 兼容；唯一兼容 → 接受）。
- 选科漂移不阻断，日志记「选科政策漂移，已忽略」。
- 累计自动 ~77%（启用归一化/子串匹配后显著提升）。
