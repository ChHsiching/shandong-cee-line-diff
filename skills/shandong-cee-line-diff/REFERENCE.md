...
> **Stage 1.5 程序模糊匹配已停用**：旧粗筛（核心名 + 括号子集消歧）曾把「临床医学(5+3一体化)」这类判错，可靠性不足；现在**所有非严格匹配的专业（约 1 万条）全部交 Stage 2 agent** 按基数规则语义判断（更准，但 agent 批次多——这是有意取舍，不惜 token 求准确，见 SKILL §3）。
