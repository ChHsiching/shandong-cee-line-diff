...
- **剥掉描述性括号**（不影响身份的说明文字）：体检要求（身高/体重/色盲/色弱/视力/体检标准）、语种、单科成绩、年龄、学费、章程引用——剥掉后再比。**不剥**：性别（男/女）、**培养标签班（拔尖/卓越/试验班等「XX 班」）**、出国模式、方向——这些是区分不同专业的标志（学校是否单独招生决定的，见下文基数规则），剥了会把不同专业判成同一个。
...
> **Stage 1.5 程序匹配（past=1 自动配）**：同校同类别同核心名、且往年只有这一条的，程序直接配（约 5000 条，构造确定、免二次复核）；同校跨类别同核心名也只剩一条时同样直接配。旧的多候选「括号子集消歧」粗筛（曾把「临床医学(5+3一体化)」判错）已停用——**只有 2 个及以上候选、或 0 候选的专业才往后走**：2+ 候选交 Stage 2 agent 按基数规则语义判断，0 候选走新增专业估算（不惜 token 求准确，见 SKILL §3）。

### agent 语义匹配的批次管理（Stage 2）

- 对严格匹配没命中的专业，按 `(校名, 招生类别)` + 核心名预筛候选；核心名兼容 = **完全相同**、或**大类↔具体（X↔X类）**，不做任意子串包含（否则「化学」会误配「化学工程与工艺」）。
...
| `--one-line <年=分,...>` | 自动从「说明」sheet 读（今年 2023=443,2024=444,2025=441） | 一段线（不传则自动读，读不到退到 `constants.ONE_LINE`） |
...
PYTHONPATH=$P "$P/.venv/bin/python" -m scripts.run_pipeline --data-dir data --out-dir output \
    ...
    --one-line 2023=439,2024=442,2025=440 \
