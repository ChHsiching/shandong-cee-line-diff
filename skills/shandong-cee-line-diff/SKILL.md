...
> **匹配由 agent 语义判断**（Stage2 agent 语义匹配）：agent 读今年专业名 + 近三年候选，**语义相同就匹配**，无需逐条问用户、无需程序硬编码。下面的六要素/忽略类/分流定则是 **agent 的判断指导**（领域知识），帮 agent 判断，不是 if-else 程序。agent **一条一条认真思考**（数据量大时可派 subagent 并行看）；**真不确定的极少数 → AskUserQuestion 问用户，问完修缮 skill 确保同类不再疑惑**（这是唯一该问用户的时候）。
...
4. **独立校区**——**需 web-search 辨别**：独立招生的分校/校区 = 不同专业（如华北电力大学保定校区 ≠ 北京校区、东北大学秦皇岛分校 ≠ 东北大学）；**同校分校区（一起招生，仅部分院系分散）= 同一专业**（如烟台大学主校区 = 八角湾校区）。不确定时 web-search 该校招生章程确认是否独立招生。
...
- **agent 语义匹配注意**：大类专业第二个括号常是「专业清单」（计算机/软件工程…），别被字面子串误导（如量子科技↔信息类字面像但语义不同）——agent 看真正方向语义判断，不靠字面子串。
- **残留分流定则（用户敲定，单一匹配真相源，少问多判）**——先剥扩充忽略类（章程/就业/校区/办学地点/培养模式/学费/体检细节/性别细节/专项类别）后判：
  - **匹配**：往年该专业只有 1 个（不管今年 1 个还是分多方向，都同一专业，如不带方向↔菁英班、实验班↔实验班、金融学↔金融学(两方向)）**或** 往年多个但今年某**方向括号内容**与往年某方向括号内容**相同/对应**（**单方向括号对应，只比方向括号、不比专业清单括号**——如量子科技先锋计划↔量子科技先锋计划、电子信息实验班↔电子信息实验班、图灵班↔图灵班(拆方向)、5+3一体化儿科学↔5+3一体化,儿科学）。
  - **估算**：往年完全没有这个专业（真新，同校同选科均值）。
  - **agent 语义**：往年多个 + 今年对不上 / 宽松子串 / 同义（数智↔数字）。agent 配对的配对，**真配不上才作特殊情况待人工——应很少**。
  - 「明显同一」构造确定，不进复核、不判存疑。
