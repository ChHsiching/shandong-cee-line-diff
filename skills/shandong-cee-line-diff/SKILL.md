...
- **残留分流定则（用户敲定，少问多判）**：
  - ① **往年该专业只有 1 个 → 今年分出新方向 → 匹配**（同一专业，如金融学↔金融学(两方向)、图灵班↔图灵班(智算/智能可信)）。
  - ② **往年完全没有这个专业 → 估算**（真新专业，同校同选科均值）。
  - ③ **其余（往年多候选对不上 / 1↔1 方向不同 / 同义如数智↔数字）→ agent 语义判定**：agent 能匹配的匹配上，**真匹配不上的才作特殊情况待人工——应很少**（如大类双方具体方向都对不上、新方向无对应）。
  - 「明显同一（标点/词序/不带方向→多方向）」构造确定，直接匹配、不进复核、不送 agent。
