...
- **可靠自动匹配（构造确定，不进复核，仅两类）**：①方向归一化**字符集相等**（标点/词序/措辞差，如「5+3一体化,儿科学」↔「5+3一体化儿科学」、「智能语言实验班」↔「语言智能实验班」）②**去年该核心名仅 1 条且无方向(bare)→今年任何方向都匹配**（金融学/法学规则）。实证 1129/2442。
- **禁宽松子串自动匹配**：大类专业第二个括号是「专业清单」（计算机/软件工程…），宽松子串会拿它**假匹配**（反例：北航「量子科技先锋计划」近三年无量子，却被专业清单子串误判应匹配）。故**拆括号/子串/同义/大类具体方向 → 交 agent 语义判定**（匹配则留主表/不匹配则待人工），**不自动**。
- **残留分流定则（用户敲定，按往年候选数 + 是否精确对上，少问多判）**：
  - **匹配**：往年该专业只有 1 个（不管今年 1 个还是分多方向，都同一专业，如不带方向↔菁英班、实验班↔实验班）**或** 往年虽多个但今年方向**精确对上其中一个**（标点/词序/措辞容差，如「智能语言实验班↔语言智能实验班」、「5+3一体化儿科学↔5+3一体化,儿科学」）。
  - **估算**：往年完全没有这个专业（真新，同校同选科均值）。
  - **agent 语义**：往年多个 + 今年对不上 / 宽松子串（量子科技↔信息类，不可靠）/ 同义（数智↔数字）。agent 能配对的配对，**真配不上才作特殊情况待人工——应很少**。
  - 「明显同一」不进复核、不判存疑；构造确定直接匹配。
