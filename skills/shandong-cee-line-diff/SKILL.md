...
- **拆括号/子串匹配（去年1→今年多=同一批）**：核心名同、去年仅1方向、今年拆多括号（金融学↔金融学(两方向)、图灵班↔图灵班(智算/智能可信)）→ 2026 各方向含/被含候选方向即匹配。
- **明显同一不得判复核存疑**：上述两类构造确定；二次复核前先跑归一化+子串匹配，构造确定直接通过，不送复核。残留真·新班级（近三年无对应）→ 新方向估算；语义近似（航天↔宇航）→ agent。
