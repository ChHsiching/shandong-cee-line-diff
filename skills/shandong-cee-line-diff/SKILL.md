...
- **残留分流定则（用户敲定，按往年候选数，少问多判）**：
  - **往年该专业只有 1 个 → 匹配**（不管今年 1 个还是分多个方向，都是同一专业；如不带方向↔菁英班、金融学↔金融学(两方向)、实验班↔实验班）。
  - **往年完全没有这个专业 → 估算**（真新专业，同校同选科均值）。
  - **往年有多个 → agent 语义判定**：对得上其中一个就匹配（如「图灵班↔图灵实验班」、标点/词序差异可直接确认），**对不上才作特殊情况待人工——应很少**（如大类双方具体方向都对不上、新方向无对应）。
  - 即「明显同一」不进复核、不判存疑；构造确定直接匹配。
