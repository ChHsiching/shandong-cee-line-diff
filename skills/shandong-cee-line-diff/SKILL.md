...
> **匹配由 agent 语义判断**（Stage2 agent 语义匹配）：agent 读今年专业名 + 近三年候选，**语义相同就匹配**，无需逐条问用户、无需程序硬编码。下面的六要素/忽略类/分流定则是 **agent 的判断指导**（领域知识），帮 agent 判断，不是 if-else 程序。agent 语义匹配完仍真不确定的极少数，才走下面的边界处理。
