...
## 匹配规则（核心领域知识——这是理解专业名匹配的全部依据）

### 什么使同名专业是「不同专业」（六要素差异化，必须对齐）

同名同校的专业，以下 **6 种括号内容差异 = 不同专业**，匹配时必须对齐：

1. **专业方向/描述**（如 `量化投资`、`含电力储能方向`）——核心名同但方向不同 = 不同专业。
   **教学样例**：`投资学(量化投资)` ≠ `投资学`——两个不同专业、不同分数线，不能混配。
2. **性别**（男/女）——有的专业分性别招生，用不同分数线。**完全同名但不同性别 = 两个独立专业**，分开匹配。
3. **校外合作**（中外合作办学 / 校企合作 / 合作国别）——不同合作轨道 = 不同专业。
4. **独立校区**（如 `东北大学秦皇岛分校` ≠ `东北大学`）——不同校区 = 不同专业。
5. **招生类别**（普通计划 / 中外合作 / 地方专项 / 高校专项 / 走读 / 边防预科…）——不同招生轨道 = 不同专业。
   - 近三年把招生类别**拼进校名**（如「三亚学院(中外合作办学)」），大绿本在**小标题(B)列**——须剥校名后缀对齐。
6. **核心名本身**（去括号后的基础专业名）——核心名不同 = 不同专业。

### 什么不影响「是否同一专业」（忽略类，剥除后匹配）

以下括号内容是**考生客观要求**（非专业身份），即使两年要求不同也不是不同专业——匹配时**剥除**：

身高 / 体重 / 色盲 / 色弱 / 视力 / 体检标准 / 合格 / 语种 / 单科成绩 / 年龄 / 特殊类型招生控制线

**性别是唯一例外**（见六要素第 2 条）——不能忽略。

### 关键洞察（影响匹配策略）

- **大绿本 2026 专业名远比近三年详细**：大绿本常在括号里塞子专业清单/校区/培养模式描述/国际交流长文；近三年是简短版。→ **核心名 + 差异化括号对齐即可匹配**，多余描述性长文不影响。
- **核心名唯一也可能错配方向**（投资学反例）→ **所有判断型匹配（粗筛+语义）须经二次 agent 复核**（precision-first）；只有严格精确匹配（归一化全名一致）才算构造确定、直接入主表。
- **选科政策漂移**：2023→2025 大量专业选科从「物理」变「物理和化学」（37.5% 近三年行跨年多值）→ **选科不作为差异化项**；匹配时选科差异不阻断，日志记「选科政策漂移，已忽略」。
