...
> **管线/审计/常量/故障排查的详细参考**：`REFERENCE.md`（按需查阅）。
...
- **验证列布局**：读各表 header，确认列索引与 `constants.py` 一致。若列变 → 更新 constants + 告知用户。
...
**主表 5 结构化列**（便于筛选）：匹配阶段 / 单年数据(是/空) / 选科漂移(是/空) / 复核结果(确定/空) / 原因备注(自由文本)。边界表保留原单一日志列。
...
> 管线阶段详细、审计检查项、常量速查、故障排查见 **`REFERENCE.md`**。
