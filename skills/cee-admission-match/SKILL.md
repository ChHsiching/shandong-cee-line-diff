...
description: 把山东省高考大绿本本科专业与近三年录取线差按专业名语义一一对应，产出带线差与日志的整理表与边界表。年度可复用。
---

# 山东省高考录取数据匹配整理（年度复用 skill 草稿）

> **草稿状态**：本文件由 Slice 7 Task 7.2 生成，待主会话用 `writing-great-skills`
> skill 精修（结构化 frontmatter / 触发条件 / 渐进式细节）。内容已覆盖全部稳定步骤。
