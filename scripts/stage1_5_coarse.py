...
def match_coarse(
    ...
    for d in unmatched:
        ...
        # 3) 跨招生类别·一对多兜底：同类别无候选，但同校同核心只有 1 条（在别的招生
        #    类别）——past=1 绝对吸收（用户口径：往年只有一个、无论什么名头/方向，
        #    今年直接用它的分）。招生类别虽是硬身份（不同专业、不同分数线），但往年
        #    只此 1 条，按一对多兜底配上、线差仅作参考；reason 必须带【跨招生类别·
        #    一对多兜底】标记，方便未来一刀切改全估算。
        any_cat = [
            h
            for h in school_idx.get(d.get("school", ""), [])
        ...
        if len(any_cat) == 1:
            ...
            mh = any_cat[0]
            hist_cat = mh.get("school_cat", "") or "普通"
            dl_cat = dl_cat_raw or "普通"
            note = (
                f"（【跨招生类别·一对多兜底】往年{d.get('school', '')}该专业只招过 "
                f"1 条记录、招生类别是「{hist_cat}」（线差 J={mh.get('J')}）；"
                f"今年这条是「{dl_cat}」。招生类别不同=不同专业、不同分数线，"
                f"但往年只此 1 条，按一对多兜底配它、J 仅作参考，不等于今年该类别"
                f"的真实门槛。）"
            )
            if "综合评价" in dl_cat or "综合评价" in (d.get("major", "") or ""):
                note += (
                    "另：今年是综合评价招生=高考分+大学校测+学业考复合录取，"
                    "往年只有普通批=纯高考分录取，两类线差口径不同。"
                )
