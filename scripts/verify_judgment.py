...
def _requirement_text(
    ...
    - ``past_same_core <= 1``（一对多）: 往年同核心只 1 个 = 该校只招了 1 个
      专业 → 今年任何**校内**变体（培养标签班 / 方向措辞 / 中外合作 / 性别 /
      标点括号）都和这 1 个往年专业是同一专业 → 判确定。
    - ``past_same_core > 1``: 往年同核心有多个 = 该校把它们当多个单独招生的
      专业（各有独立分数线）；agent 已从中选了 1 个。身份由数据定、不靠标签:
      培养标签班/出国模式/中外合作/师范/招生类别/学制/独立划线校区/真方向
      实质不同 → 判存疑；只有纯描述噪音差异 → 判确定。

    注：身份由「该校往年有没有分多条记录招」决定，不靠标签硬判——培养标签班
    (拔尖/卓越/试验班等) 在 past>1 时是不同专业（学校分线招了）、在 past=1 时
    被一对多吸收。校区分两种：同一所大学的校区标记（高密/舜耕校区）= 同一专业
    （描述噪音，→确定）；但「校区」其实是各自独立划线的不同办学点（如中山
    广州/珠海/深圳）或本就两所不同校（华电北京≠华电保定，已在匹配前按校名
    分开）= 不该出现在同校配对里、跨配=配错。
    """
    common_tail = (
        "大类↔具体(今年 X ↔ 往年 X类, cores 仅差『类』后缀)属一对多: 往年大类"
        "确实覆盖今年这个具体方向(如 工商管理↔工商管理类、护理学↔护理学类、"
        "财政学↔财政学类)→判确定(大类线差作参考); 工科试验班类这种混杂宽大类"
        "对不上→存疑。"
        "真正不同的专业(如投资学(量化投资)≠投资学、不同招生类别、跨学制)才存疑。"
        "同一所大学的校区标记(高密/舜耕校区)=同一专业→确定; 但独立划线的不同办学点"
        "(如中山 广州/珠海/深圳)或两所不同校(华电北京≠华电保定)→不该跨配, 跨了=配错"
        "→存疑。不确定→存疑, 宁可保守。"
    )
    if past_same_core <= 1:
        return (
    ...
    return (
        "判定该配对是否确定正确。往年同核心专业有多个 = 该校把它们当多个单独招生"
        "的专业(各有独立分数线); agent 已从中选了 1 个。身份由数据定、不靠标签: "
        "培养标签班(拔尖/卓越/创新/英才/基地/未来/试验班/订单班等)/出国模式/中外"
        "合作/师范/招生类别/学制/独立划线校区/真方向 实质不同 → 判存疑; 只有纯描述"
        "噪音(体检/色盲/学费/语种/章程/词序/标点)差异 → 判确定。"
        + common_tail
    )
