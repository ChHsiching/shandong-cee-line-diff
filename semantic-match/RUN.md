...
- `prompt`：读 `semantic-match/prompt.md`（六要素规则 + 输出 schema）作为系统
  指令，再把 `batch_NN_prompt.json` 的 `items` 作为输入，要求 agent 对每条
  item 输出一行 JSON 到 `semantic-match/batch_NN_result.jsonl`，字段严格遵循
  `output_schema`。
